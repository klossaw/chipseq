...
lapply(samples, \(samp){
  for(ip in ips) {
    glue("{samp}/peaks_annotation/ip_{ip}") %>% fs::dir_create()
    # read the peaks and plot the region coverage
    peak <- readPeakFile(glue("{samp}/peak_calling/{samp}_{ip}.narrowPeak"))
    p <- covplot(peak, weightCol = "V5", chrs = paste0("chr", c(1:22, "X", "Y"))) + 
      theme(text = element_text(size = 6))
    ggsave(glue("{samp}/peaks_annotation/ip_{ip}/peak_region_coverage_each_chr.pdf"), p, width = 8, height = 12)
    # annotation
    ...
    readr::write_tsv(peak_anno_tbl, glue("{samp}/peaks_annotation/ip_{ip}/peaks_annotation_tbl.xls"))
    # plots
    pdf(glue("{samp}/peaks_annotation/ip_{ip}/peak_annot_pie.pdf"))
    ...
    tss_dist_p <- plotDistToTSS(peak_anno, title = "Distribution of transcription factor-binding loci relative to TSS") + 
      theme(text = element_text(size = 6))
    ggsave(glue("{samp}/peaks_annotation/ip_{ip}/tss_dist_bar.pdf"), tss_dist_p, width = 4, height = 2)
    ...
    readr::write_rds(peak_enrich_lst, glue("{samp}/peaks_annotation/ip_{ip}/peak_annot_enrichment_GO.rds"))
    ...
    write_rds(peak_enrich_lst[["kegg"]], glue("{samp}/peaks_annotation/ip_{ip}/peak_annot_enrichment_kegg.rds"))
    ...
    parallel::mclapply(c("BP", "CC", "MF", "kegg"), \(ctg){
      try({
        ...
        if(ctg != "kegg") {
          ggsave(glue("{samp}/peaks_annotation/ip_{ip}/peak_anno_enrichment_GO_{ctg}.pdf"), p, width = 4, height = 4)
        } else {
          ggsave(glue("{samp}/peaks_annotation/ip_{ip}/peak_anno_enrichment_{ctg}.pdf"), p, width = 4, height = 4)
        }
        readr::write_tsv(peak_enrich_lst[[ctg]]@result, glue("{samp}/peaks_annotation/ip_{ip}/enrich_res_{ctg}.xls"))
      })
    }, mc.cores = 4)
